...
salaryData_Train <- read.csv("D:/R Excel Sessions/Assignments/Support vector machines/SalaryData_Train.csv")
salaryData_Test <- read.csv("D:/R Excel Sessions/Assignments/Support vector machines/SalaryData_Test.csv")
...
forestfires_data <- read.csv("D:/R Excel Sessions/Assignments/Support vector machines/forestfires.csv")
...
###As per variable importance we can remove variables like area,monthapr,monthdec,monthjun,monthmay,monthoct
...
pca_data <- cbind(forest_scaled,forest_newdata[,29])
...
pca_data2 <- rename(pca_data,c(`forest_newdata[, 29]`="size_category"))
...
######Run SVM algorithm on first 4 PC scores


library(caret)
intraininglocal <- createDataPartition(pca_data3$size_category,p=0.7,list = F)

forest_training <- pca_data3[intraininglocal,]
forest_Testing <- pca_data3[-intraininglocal,]

dim(forest_training)
dim(forest_Testing)

colnames(forest_training)

library(kernlab)

Forest_classifier_withPCA <- ksvm(size_category~Comp.1+Comp.2+Comp.3+Comp.4,data=forest_training,kernel = "rbfdot")
forest_predictions <- predict(Forest_classifier_withPCA,forest_Testing)
